...
namespace SquareCore
{
    ...
    void EntityManager::SetColliderEnabled(uint32_t entityID, bool enabled)
    {
        std::lock_guard<std::mutex> lock(entityMutex);
        ...
        if (it != idToIndex.end())
        {
            entities[it->second].collider.enabled = enabled;
            ...
            else if (physicsRef && !entities[it->second].physicsHandle.isValid && enabled && entities[it->second].collider.type
                != ColliderType::NONE)
            {
                entityMutex.unlock();
                physicsRef->CreateBody(entityID);
        ...
        {
            SDL_LogError(SDL_LOG_CATEGORY_ERROR, "SetColliderEnabled: Entity ID %u not found", entityID);
        }
    }

    void EntityManager::SetColliderOffset(uint32_t entityID, const Vec2& offset)
    {
        std::lock_guard<std::mutex> lock(entityMutex);
        ...
        if (it != idToIndex.end())
        {
            entities[it->second].collider.offset = offset;
        ...
        {
            SDL_LogError(SDL_LOG_CATEGORY_ERROR, "SetColliderOffset: Entity ID %u not found", entityID);
        }
    }

    void EntityManager::SetColliderSize(uint32_t entityID, const Vec2& size)
    {
        std::lock_guard<std::mutex> lock(entityMutex);
        ...
        if (it != idToIndex.end())
        {
            entities[it->second].collider.size = size;
        ...
        {
            SDL_LogError(SDL_LOG_CATEGORY_ERROR, "SetColliderSize: Entity ID %u not found", entityID);
        }
    }
