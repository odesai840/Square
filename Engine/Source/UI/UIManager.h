...
namespace SquareCore
{
    ...
    {
    public:
        uint32_t AddRect(float x_pos, float y_pos, float width, float height, RGBA color);
        uint32_t AddButton(float x_pos, float y_pos, float width, float height, RGBA color, std::function<void()> onPress = nullptr);
